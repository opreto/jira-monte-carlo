...
class JiraCSVParser:
    ...
    def _create_issue_from_row(self, row: Dict) -> Optional[Issue]:
        try:
            ...
            story_points = self._parse_float(
                row.get(self.field_mapping.story_points_field)
            ) if self.field_mapping.story_points_field else None
            
            time_estimate = self._parse_float(
                row.get(self.field_mapping.time_estimate_field)
            ) if self.field_mapping.time_estimate_field else None
            
            time_spent = self._parse_float(
                row.get(self.field_mapping.time_spent_field)
            ) if self.field_mapping.time_spent_field else None
