...
class EntityMapper:
    ...
    def map_velocity_metrics(self, metrics: VelocityMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            'coefficient_of_variation': round(metrics.std_dev / metrics.average, 3) if metrics.average > 0 else 0
        }
    ...
    def map_health_metrics(self, metrics: ProcessHealthMetrics) -> Dict[str, Any]:
        ...
        if metrics.aging_analysis:
            result['components']['aging'] = self._map_aging_analysis(metrics.aging_analysis)
        
        if metrics.wip_analysis:
            result['components']['wip'] = self._map_wip_analysis(metrics.wip_analysis)
        
        if metrics.sprint_health:
            result['components']['sprint_health'] = self._map_sprint_health(metrics.sprint_health)
        
        if metrics.blocked_analysis:
            result['components']['blocked'] = self._map_blocked_analysis(metrics.blocked_analysis)
        
        if metrics.lead_time_analysis:
            result['components']['lead_time'] = self._map_lead_time_analysis(metrics.lead_time_analysis)
    ...
    def _map_sprint_health(self, health: SprintHealthAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_velocity': round(health.average_velocity, 1) if health.average_velocity else None,
            'average_scope_change': round(health.average_scope_change * 100, 1) if health.average_scope_change else None,
            'score': round(health.health_score * 100, 1),
            'completion_by_sprint': {
                sprint: round(rate * 100, 1)
                for sprint, rate in list(health.completion_by_sprint.items())[-5:]  # Last 5 sprints
            }
        }
