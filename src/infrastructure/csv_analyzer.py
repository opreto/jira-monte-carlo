...
class EnhancedSprintExtractor:
    ...
    def extract_from_dataframe(
        df: pl.DataFrame, sprint_column: str, status_column: str, done_statuses: List[str], story_points_column: str
...
class VelocityExtractor:
    ...
    def extract_velocity_data(
        ...
        2. Too many sprints share the same date (>20% duplicates)
        """
        ...
        if has_sprint_column and has_date_column:
            ...
            if sprint_dates_map:
                ...
                for i in range(1, len(sorted_sprint_data)):
                    prev_date = sorted_sprint_data[i - 1][1][0]
                    curr_date = sorted_sprint_data[i][1][0]
                ...
                if clustering_ratio > 0.3:
                    logger.info(
                        ...
                        "Using synthetic dates for cleaner visualization."
                    )
                    use_synthetic = True
                else:
                    # Use real dates
                    for sprint_name, (sprint_date, data) in sprint_dates_map.items():
