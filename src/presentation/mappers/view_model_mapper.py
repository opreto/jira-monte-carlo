...
class ViewModelMapper:
    ...
    def create_forecast_summary(
        ...
        summary_cards = [
            self.create_metric_card(
                "Remaining Work",
                simulation_config.remaining_work,
                "points"
            ),
            self.create_metric_card(
                "Average Velocity",
            ...
            self.create_metric_card(
                ...
                "%",
                color="success" if velocity_metrics.std_dev < velocity_metrics.average * 0.2 else "warning"
            )
        ]
    ...
    def create_health_score(
        ...
        if health_metrics.wip_analysis:
            components.append(
                self._create_wip_component(health_metrics.wip_analysis)
            )
    ...
    def create_summary_table(
        ...
        # Add header row
        rows.append(TableRowViewModel(
            cells=["Confidence Level", "Sprints to Complete", "Completion Date", "Probability"],
            is_header=True
        ))
        ...
        for confidence, label, css_class in [
            ...
            if confidence in simulation_result.percentiles:
                ...
                date = datetime.now()
                # Would add days to date
                date_str = date.strftime("%Y-%m-%d")
                
                probability_str = f"{int(confidence * 100)}% chance of completing by this date"
    ...
    def _assess_forecast_risk(
        ...
        """
        # Calculate spread
        if 0.95 in simulation_result.percentiles and 0.5 in simulation_result.percentiles:
            spread = simulation_result.percentiles[0.95] - simulation_result.percentiles[0.5]
            relative_spread = spread / simulation_result.percentiles[0.5]
        else:
            relative_spread = 0
        
        # Calculate velocity stability
        velocity_cv = velocity_metrics.std_dev / velocity_metrics.average if velocity_metrics.average > 0 else 0
    ...
    def _create_aging_component(
        ...
        recommendations = []
        if aging.abandoned_count > 0:
            recommendations.append(f"Review and close {aging.abandoned_count} abandoned items")
        if old_ratio > 0.2:
    ...
    def _create_sprint_health_component(
        ...
        recommendations = []
        if score < 80:
            recommendations.append("Improve sprint planning to increase completion rate")
        if sprint_health.average_scope_change and abs(sprint_health.average_scope_change) > 0.2:
            recommendations.append("High scope change detected - stabilize sprint commitments")
