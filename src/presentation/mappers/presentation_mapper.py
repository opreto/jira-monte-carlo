...
class PresentationMapper:
    ...
    def create_full_report_view_model(
        ...
        sim_result = forecast_result.simulation_results.get(
            ...
            list(forecast_result.simulation_results.values())[0]
        )
        ...
        # Create charts
        chart_preferences = self.request_response_mapper.extract_chart_preferences(
            report_request
        ) if report_request else {
        ...
        if health_metrics and report_request and report_request.include_health_metrics:
            ...
            if health_metrics.wip_analysis:
                report_vm.health_charts['wip_by_status'] = \
                    self.chart_mapper.create_wip_by_status_chart(health_metrics.wip_analysis)
            
            if health_metrics.sprint_health:
                report_vm.health_charts['sprint_completion'] = \
                    self.chart_mapper.create_sprint_completion_chart(health_metrics.sprint_health)
        
        # Add methodology description
        report_vm.methodology_description = (
            f"Monte Carlo simulation with {forecast_result.monte_carlo_config.num_simulations:,} iterations"
        )
        report_vm.simulation_count = str(forecast_result.monte_carlo_config.num_simulations)
    ...
    def create_dashboard_view_model(
        ...
        avg_velocity = sum(p.get('velocity', 0) for p in projects) / len(projects) if projects else 0
