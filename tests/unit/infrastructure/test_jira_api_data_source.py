...
class TestJiraApiDataSource:
    ...
    def test_parse_with_cache_hit(self, jira_data_source):
        ...
        # Setup cache hit
        cached_issues = [
            Issue(key="TEST-1", summary="Test Issue", issue_type="Story", 
                  status="Done", created=datetime.now(timezone.utc), 
                  reporter="test@example.com")
        ]
        cached_sprints = [
            Sprint(name="Sprint 1", start_date=datetime.now(timezone.utc), 
                   end_date=datetime.now(timezone.utc), completed_points=10.0)
        ]
