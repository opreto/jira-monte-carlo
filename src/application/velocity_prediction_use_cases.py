...
from ..domain.velocity_adjustments import (
    VelocityScenario, VelocityAdjustment, TeamChange, ScenarioComparison
)
...
class ApplyVelocityAdjustmentsUseCase:
    ...
    def _create_adjusted_metrics(
        ...
        return VelocityMetrics(
            ...
            percentile_75=base_metrics.percentile_75 * avg_factor,
            percentile_90=base_metrics.percentile_90 * avg_factor,
...
class GenerateScenarioComparisonUseCase:
    ...
    def execute(
        ...
        # Get key percentiles
        baseline_p50 = baseline.get_percentile(50)
        baseline_p85 = baseline.get_percentile(85)
        adjusted_p50 = adjusted.get_percentile(50)
        adjusted_p85 = adjusted.get_percentile(85)
        
        # Calculate velocity impact
        baseline_avg_velocity = baseline.mean_sprints
        adjusted_avg_velocity = adjusted.mean_sprints
        velocity_impact = ((adjusted_avg_velocity - baseline_avg_velocity) / 
                          baseline_avg_velocity * 100)
...
class CreateVelocityScenarioUseCase:
    ...
    def execute(
        ...
        sorted_adjustments = sorted(
            ...
            key=lambda a: a.sprint_start
        )
        sorted_changes = sorted(
            team_changes,
            key=lambda c: c.sprint
        )
        
        return VelocityScenario(
            name=name,
            adjustments=sorted_adjustments,
            team_changes=sorted_changes
        )