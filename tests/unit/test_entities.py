...
class TestIssue:
    def test_cycle_time_calculation(self):
        ...
        issue = Issue(
            key="TEST-1", summary="Test issue", issue_type="Story", status="Done", created=created, resolved=resolved
        )
    ...
    def test_cycle_time_unresolved(self):
        issue = Issue(
            key="TEST-1", summary="Test issue", issue_type="Story", status="In Progress", created=datetime(2023, 1, 1)
        )
    ...
    def test_age_calculation(self):
        ...
        issue = Issue(key="TEST-1", summary="Test issue", issue_type="Story", status="In Progress", created=created)
...
class TestSprint:
    def test_velocity_calculation(self):
        sprint = Sprint(
            name="Sprint 1", start_date=datetime(2023, 1, 1), end_date=datetime(2023, 1, 14), completed_points=42.0
        )
...
class TestTeam:
    def test_average_velocity(self):
        team = Team(name="Alpha Team", members=["Alice", "Bob"], historical_velocities=[20.0, 30.0, 40.0])
