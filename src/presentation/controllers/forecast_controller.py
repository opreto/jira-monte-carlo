...
class ForecastController(Controller[ForecastRequest, ForecastResponse]):
    ...
    def handle(self, request: ForecastRequest) -> ForecastResponse:
        ...
        try:
            ...
            velocity_metrics = self.velocity_use_case.execute(
                request.lookback_sprints, request.velocity_field
            )
            ...
            dates = self._calculate_completion_dates(
                sim_result.percentiles, request.sprint_length, base_date
            )
            ...
            response.risk_assessment = self._assess_risk(
                velocity_metrics, sim_result.percentiles
            )
            response.recommendations = self._generate_recommendations(
                velocity_metrics, remaining_work, sim_result.percentiles
            )
    ...
    def _calculate_completion_dates(
        self, percentiles: Dict[float, float], sprint_length: int, base_date: datetime
    ) -> Dict[int, datetime]:
    ...
    def _assess_risk(self, velocity_metrics, percentiles: Dict[float, float]) -> str:
        ...
        relative_spread = (
            spread / percentiles.get(0.5, 1) if percentiles.get(0.5, 0) > 0 else 0
        )

        # Assess based on spread and velocity stability
        if (
            relative_spread > 0.5
            or velocity_metrics.std_dev > velocity_metrics.average * 0.3
        ):
            return "High uncertainty - Wide spread between optimistic and pessimistic estimates"
        elif (
            relative_spread > 0.3
            or velocity_metrics.std_dev > velocity_metrics.average * 0.2
        ):
    ...
    def _generate_recommendations(
        self, velocity_metrics, remaining_work: float, percentiles: Dict[float, float]
    ) -> List[str]:
