...
class TeamChange:
    ...
    def get_description(self) -> str:
        ...
        if self.change > 0:
            return f"Adding {self.change} developer{'s' if self.change > 1 else ''} starting sprint {self.sprint} (ramp-up: {self.ramp_up_sprints} sprints)"
        else:
            return f"Removing {abs(self.change)} developer{'s' if abs(self.change) > 1 else ''} after sprint {self.sprint}"
