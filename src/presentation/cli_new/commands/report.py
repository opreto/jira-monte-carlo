...
class ReportCommand(Command):
    ...
    def __init__(self, console: Optional[Console] = None):
        super().__init__(
            name="report", description="Generate HTML reports from forecast results"
        )
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            if "velocity_scenarios" in context.dependencies:
                report_context["velocity_scenarios"] = context.dependencies[
                    "velocity_scenarios"
                ]
                report_context["scenario_comparison"] = context.dependencies.get(
                    "scenario_comparison"
                )
            ...
            return CommandResult(
                success=False, message=f"Report generation failed: {str(e)}", error=e
            )

    def _analyze_process_health(
        self, context: CommandContext, issues: list, sprints: list, args: Any
    ) -> Dict[str, Any]:
        """Run all process health analyses"""
        results = {}
            ...
            breakdown.append(
                {
                    ...
                    "score": 1.0
                    - (
                        (aging.stale_count + aging.abandoned_count)
                        / max(aging.total_items, 1)
                    ),
