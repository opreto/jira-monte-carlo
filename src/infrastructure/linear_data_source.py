                ...
                if issue:
                    ...
                    if cycle_name and cycle_name.strip():
                        ...
                        if self._is_done_status(row.get(self.field_mapping.status_field)):
                            ...
                            if completed_date:
                                if not cycles[cycle_name]["end_date"] or completed_date > cycles[cycle_name]["end_date"]:
                                    cycles[cycle_name]["end_date"] = completed_date
                                if not cycles[cycle_name]["start_date"] or completed_date < cycles[cycle_name]["start_date"]:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                linear_indicators = [
                    ...
                    'Assignee', 'Labels', 'Created', 'Updated', 'Completed',
                    'Cycle', 'Project', 'Team', 'Parent', 'URL',
                    'Lead Time', 'Cycle Time'
                ]
                
                # Count matches
                matches = sum(1 for field in linear_indicators 
